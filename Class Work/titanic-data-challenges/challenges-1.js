...
const getSurvivorCount = (data) => {
  return data.reduce((ans, passenger) => {
    return ans + (passenger.fields.survived === "Yes" ? 1 : 0)
  }, 0)
}
...
const getCasualityCount = (data) => {
  return data.reduce((ans, passenger) => {
    return ans + (passenger.fields.survived === "No" ? 1 : 0)
  }, 0)
}
...
const countPassengersInClass = (data, pclass) => {
  return data.reduce((ans, passenger) => {
    return ans + (passenger.fields.pclass === pclass ? 1 : 0)
  }, 0)
}
...
const getMinAge = (data) => {
  return data
    .filter((passenger) => !isNaN(passenger.fields.age))
    .reduce((minAge, passenger) => (minAge = Math.min(minAge, passenger.fields.age)), Infinity)
}
...
const getMaxAge = (data) => {
  return data
    .filter((passenger) => !isNaN(passenger.fields.age))
    .reduce((maxAge, passenger) => (maxAge = Math.max(maxAge, passenger.fields.age)), -Infinity)
}
...
const getMinFare = (data) => {
  return data
    ...
    .reduce((minFare, passenger) => (minFare = Math.min(minFare, passenger.fields.fare)), Infinity)
}
...
const getPassengersByGender = (data, gender) => {
  return data.reduce((ans, passenger) => {
    if (passenger.fields.sex === gender) {
      return ans + 1
    }
    return ans
  }, 0)
...
const getSurvivorsByGender = (data, gender) => {
  return data.reduce((ans, passenger) => {
    if (passenger.fields.sex === gender && passenger.fields.survived === "Yes") {
      return ans + 1
    }
    return ans
  }, 0)
...
const getCasualitiesByGender = (data, gender) => {
  return data.reduce((ans, passenger) => {
    if (passenger.fields.sex === gender && passenger.fields.survived === "No") {
      return ans + 1
    }
    return ans
  }, 0)
...
const getTotalFare = (data) => {
  return data
    .filter((passenger) => !isNaN(passenger.fields.fare) && passenger.fields.fare !== -1)
    .reduce((totalFare, passenger) => totalFare + passenger.fields.fare, 0)
}
...
const getAverageFare = (data) => {
  const filteredPassengers = data.filter(
    (passenger) => !isNaN(passenger.fields.fare) && passenger.fields.fare !== -1
  )
  const totalFare = filteredPassengers.reduce(
    (totalFare, passenger) => totalFare + passenger.fields.fare,
    0
  )
  return totalFare / filteredPassengers.length
}
...
const getMedianFare = (data) => {
  ...
  let mid = fares.length / 2
  if (fares.length % 2 == 0) {
    return (fares[mid].fields.fare + fares[mid + 1].fields.fare) / 2
  } else {
    mid = Math.ceil(mid)
    return fares[mid].fields.fare
  }
}
...
const getAverageAge = (data) => {
  const filteredPassengers = data.filter((passenger) => !isNaN(passenger.fields.age))
  const totalAge = filteredPassengers.reduce(
    (totalAge, passenger) => totalAge + passenger.fields.age,
    0
  )
  return totalAge / filteredPassengers.length
}
...
const getMedianAge = (data) => {
  const ages = data
    .filter((passenger) => !isNaN(passenger.fields.age))
    .sort((a, b) => a.fields.age < b.fields.age)

  let mid = ages.length / 2
  if (ages.length % 2 == 0) {
    return (ages[mid].fields.age + ages[mid + 1].fields.age) / 2
  } else {
    mid = Math.ceil(mid)
    return ages[mid].fields.age
  }
}
...
const getAverageAgeByGender = (data, gender) => {
  const filteredPassengers = data.filter(
    (passenger) => !isNaN(passenger.fields.age) && passenger.fields.sex === gender
  )
  const totalAge = filteredPassengers.reduce(
    (totalAge, passenger) => totalAge + passenger.fields.age,
    0
